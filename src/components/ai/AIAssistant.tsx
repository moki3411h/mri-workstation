...
export default function AIAssistant() {
  ...
  return (
    <div style={{ position:'fixed', top:0, right:0, bottom:0, width:'360px', background:'#0a1220', borderLeft:'1px solid #1e3a5f', zIndex:800, display:'flex', flexDirection:'column', overflow:'hidden', boxShadow:'-4px 0 24px rgba(0,0,0,0.5)' }}>
      {/* Header */}
      <div style={{ display:'flex', alignItems:'center', padding:'10px 14px', borderBottom:'1px solid #1e293b', background:'rgba(14,165,233,0.05)', flexShrink:0 }}>
        <div>
          <div style={{ fontSize:'12px', fontWeight:700, color:'#e2e8f0' }}>🤖 AI Protocol Assistant</div>
          <div style={{ fontSize:'8.5px', color:'#475569', marginTop:'1px' }}>Educational suggestions — not clinical advice</div>
        </div>
        <button onClick={toggleAI} style={{ marginLeft:'auto', background:'transparent', border:'none', color:'#64748b', fontSize:'18px', cursor:'pointer' }}>✕</button>
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'12px 14px', display:'flex', flexDirection:'column', gap:'12px' }}>
        ...
        {/* Protocol Suggestion */}
        <div style={{ padding:'10px', background:'#0d1626', border:'1px solid #1e293b', borderRadius:'2px' }}>
          <div style={{ fontSize:'9px', fontWeight:700, color:'#22d3ee', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'8px' }}>Protocol Suggestion</div>
          <textarea
            value={input}
            onChange={e=>setInput(e.target.value)}
            placeholder="Describe clinical indication... (e.g., 'acute stroke', 'suspected glioma', 'MS follow-up')"
            style={{ width:'100%', height:'56px', background:'#060b14', border:'1px solid #263040', color:'#94a3b8', fontFamily:'Roboto Mono,monospace', fontSize:'9px', padding:'5px 8px', borderRadius:'2px', outline:'none', resize:'none' }}
          />
          <button
            onClick={()=>{ if (input.trim()) { setSuggestion(getSuggestion(input)); toast('Protocol suggestion generated','success'); } }}
            style={{ marginTop:'6px', width:'100%', padding:'5px', fontSize:'9.5px', fontWeight:600, background:'rgba(34,211,238,0.12)', border:'1px solid rgba(34,211,238,0.3)', color:'#22d3ee', cursor:'pointer', borderRadius:'2px' }}
          >
            Generate Suggestion ▶
          </button>
          {suggestion && (
            <div style={{ marginTop:'8px', padding:'8px', background:'rgba(34,211,238,0.05)', border:'1px solid rgba(34,211,238,0.15)', borderRadius:'2px' }}>
              <div style={{ fontSize:'8px', color:'#22d3ee', fontWeight:700, marginBottom:'4px' }}>SUGGESTED PROTOCOL:</div>
              <div style={{ fontSize:'9.5px', color:'#94a3b8', lineHeight:'1.55' }}>{suggestion}</div>
            </div>
          )}
        </div>
        ...
        {paramHints.length > 0 && (
          <div style={{ padding:'10px', background:'#0d1626', border:'1px solid #1e293b', borderRadius:'2px' }}>
            <div style={{ fontSize:'9px', fontWeight:700, color:'#f59e0b', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'8px' }}>⚡ Parameter Hints</div>
            {paramHints.map((h,i) => (
              <div key={i} style={{ fontSize:'9.5px', color:'#94a3b8', padding:'5px 0', borderBottom:'1px solid #0d1520', lineHeight:'1.4' }}>{h}</div>
            ))}
          </div>
        )}
        ...
        <div style={{ padding:'10px', background:'#0d1626', border:'1px solid #1e293b', borderRadius:'2px' }}>
          <div style={{ fontSize:'9px', fontWeight:700, color:'#22d3ee', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'8px' }}>Quick Protocols</div>
          <div style={{ display:'flex', flexWrap:'wrap', gap:'5px' }}>
            {PROTOCOLS.map(p => (
              <button key={p.label}
                ...
                style={{ fontSize:'9px', padding:'4px 10px', background:'#1c2a3e', border:'1px solid #263040', color:'#64748b', cursor:'pointer', borderRadius:'2px', transition:'all 0.1s' }}
                onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.color='#22d3ee';(e.currentTarget as HTMLElement).style.borderColor='#22d3ee';}}
                onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.color='#64748b';(e.currentTarget as HTMLElement).style.borderColor='#263040';}}
              >{p.label}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ padding:'10px', background:'#0d1626', border:'1px solid #1e293b', borderRadius:'2px' }}>
          <div style={{ fontSize:'9px', fontWeight:700, color:'#22d3ee', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'8px' }}>Current Session</div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'4px', fontSize:'8.5px' }}>
            <div style={{ color:'#475569' }}>Active Seq:</div><div style={{ color:'#64748b', fontFamily:'Roboto Mono,monospace' }}>{selectedSeq?.name?.slice(0,18) ?? '—'}</div>
            <div style={{ color:'#475569' }}>Calc TA:</div><div style={{ color:'#22c55e', fontFamily:'Roboto Mono,monospace' }}>{calcTA}</div>
            <div style={{ color:'#475569' }}>SNR Estimate:</div><div style={{ color: calcSNR>=60?'#22c55e':calcSNR>=30?'#f59e0b':'#ef4444', fontFamily:'Roboto Mono,monospace' }}>{calcSNR}</div>
            <div style={{ color:'#475569' }}>TR / TE:</div><div style={{ color:'#64748b', fontFamily:'Roboto Mono,monospace' }}>{params.tr} / {params.te}ms</div>
          </div>
        </div>
      </div>
    </div>
