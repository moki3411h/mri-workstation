...
const Field = memo(function Field({ label, value, onChange, type = 'text', placeholder, required, tabIndex, readOnly }: FieldProps) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', marginBottom:'8px' }}>
      <label style={{ fontSize:'8px', color:'var(--c-text-subtle)', marginBottom:'2px', letterSpacing:'0.3px', textTransform:'uppercase' }}>
        {label}{required && <span style={{ color:'var(--c-red)', marginLeft:'2px' }}>*</span>}
      </label>
      <input
        ref={ref}
        type={type}
        ...
        spellCheck={false}
        style={{
          background: readOnly ? 'var(--c-bg-deepest)' : 'var(--c-bg-input)',
          border: `1px solid ${readOnly ? 'var(--c-border)' : 'var(--c-border-bright)'}`,
          color: readOnly ? 'var(--c-text-subtle)' : 'var(--c-text-bright)',
        ...
        onFocus={e => { if (!readOnly) (e.target as HTMLInputElement).style.borderColor = 'var(--c-cyan)'; }}
        onBlur={e => { (e.target as HTMLInputElement).style.borderColor = readOnly ? 'var(--c-border)' : 'var(--c-border-bright)'; }}
      />
    </div>
  );
});
...
const SelectField = memo(function SelectField({ label, value, onChange, options, tabIndex }: SelectFieldProps) {
  return (
    <div style={{ display:'flex', flexDirection:'column', marginBottom:'8px' }}>
      <label style={{ fontSize:'8px', color:'var(--c-text-subtle)', marginBottom:'2px', letterSpacing:'0.3px', textTransform:'uppercase' }}>{label}</label>
      <select
        value={value}
        onChange={e => onChange(e.target.value)}
        tabIndex={tabIndex}
        style={{ background:'var(--c-bg-input)', border:'1px solid var(--c-border-bright)', color:'var(--c-text-bright)', fontFamily:'Roboto Mono,monospace', fontSize:'10px', padding:'5px 6px', borderRadius:'2px', outline:'none' }}
      >
...
const CheckRow = memo(function CheckRow({ label, sublabel, checked, onChange, riskLevel = 'red', tabIndex }: CheckRowProps) {
  const color = riskLevel === 'red' ? 'var(--c-red)' : riskLevel === 'amber' ? 'var(--c-amber)' : 'var(--c-green)';
  return (
    <label style={{ display:'flex', alignItems:'flex-start', gap:'10px', padding:'5px 0', borderBottom:'1px solid var(--c-border-faint)', cursor:'pointer', userSelect:'none' }} tabIndex={tabIndex}>
      <div style={{ position:'relative', width:'14px', height:'14px', flexShrink:0, marginTop:'1px' }}>
        <input type="checkbox" checked={checked} onChange={e => onChange(e.target.checked)}
          style={{ position:'absolute', opacity:0, width:0, height:0 }} />
        <div style={{
          width:'14px', height:'14px', border:`1px solid ${checked ? color : 'var(--c-border-bright)'}`,
          background: checked ? `${color}20` : 'var(--c-bg-input)', borderRadius:'2px',
      ...
      <div>
        <div style={{ fontSize:'9.5px', color: checked ? color : 'var(--c-text-mid)' }}>{label}</div>
        {sublabel && <div style={{ fontSize:'8px', color:'var(--c-text-muted)', marginTop:'1px' }}>{sublabel}</div>}
      </div>
      {checked && riskLevel !== 'green' && (
        <div style={{ marginLeft:'auto', fontSize:'7.5px', fontWeight:700, color, background:`color-mix(in srgb, ${color} 6%, transparent)`, border:`1px solid color-mix(in srgb, ${color} 20%, transparent)`, borderRadius:'2px', padding:'1px 5px', flexShrink:0 }}>
      ...
      <div className="modal-box" style={{ width: '760px', maxWidth: '96vw' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', padding:'10px 16px', borderBottom:'1px solid var(--c-border-accent)', background:'rgba(14,165,233,0.05)', flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
            ...
            <div>
              <div style={{ fontSize:'12px', fontWeight:700, color:'var(--c-text-bright)' }}>Patient Registration</div>
              <div style={{ fontSize:'8.5px', color:'var(--c-text-subtle)' }}>Register new patient — MRI Safety Clearance Required</div>
            </div>
          </div>
          <div style={{ marginLeft:'auto', display:'flex', gap:'6px', alignItems:'center' }}>
            <div style={{ position:'relative' }}>
              <input
                ...
                style={{ background:'var(--c-bg-input)', border:'1px solid var(--c-border-bright)', color:'var(--c-text-base)', fontFamily:'Roboto Mono,monospace', fontSize:'9.5px', padding:'4px 10px', borderRadius:'2px', outline:'none', width:'160px' }}
              />
            </div>
            <button onClick={togglePatient} style={{ background:'transparent', border:'none', color:'var(--c-text-mid)', fontSize:'18px', cursor:'pointer', padding:'0 4px' }}>✕</button>
          </div>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0', overflowY:'auto', maxHeight:'68vh' }}>
          ...
          <div style={{ padding:'14px 16px', borderRight:'1px solid var(--c-border)', overflowY:'auto' }}>
            <div style={{ fontSize:'9px', fontWeight:700, color:'var(--c-cyan)', textTransform:'uppercase', letterSpacing:'0.8px', marginBottom:'12px', paddingBottom:'4px', borderBottom:'1px solid var(--c-border)' }}>
              Patient Information
            </div>
            ...
            <div style={{ borderTop:'1px solid var(--c-border)', paddingTop:'10px', marginTop:'4px' }}>
              <div style={{ fontSize:'9px', fontWeight:700, color:'var(--c-cyan)', textTransform:'uppercase', letterSpacing:'0.8px', marginBottom:'10px' }}>Study Information</div>
          ...
          <div style={{ padding:'14px 16px', background:'rgba(0,0,0,0.15)', overflowY:'auto' }}>
            ...
            {(isUnsafe || isWarning) && (
              <div style={{ marginBottom:'10px', padding:'7px 10px', background:`rgba(${isUnsafe?'239,68,68':'245,158,11'},0.1)`, border:`1px solid rgba(${isUnsafe?'239,68,68':'245,158,11'},0.4)`, borderRadius:'2px' }}>
                <div style={{ fontSize:'9px', fontWeight:700, color: isUnsafe?'var(--c-red)':'var(--c-amber)' }}>
                  {isUnsafe ? '⚠ CONTRAINDICATION DETECTED' : '⚠ CAUTION — VERIFY BEFORE SCANNING'}
                </div>
                <div style={{ fontSize:'8.5px', color:'var(--c-text-base)', marginTop:'2px' }}>
                  {isUnsafe
            ...
            <div style={{ marginBottom:'6px', fontSize:'8px', color:'var(--c-text-muted)', fontWeight:700, letterSpacing:'0.3px' }}>ABSOLUTE CONTRAINDICATIONS</div>
            ...
            <div style={{ marginTop:'10px', marginBottom:'6px', fontSize:'8px', color:'var(--c-text-muted)', fontWeight:700, letterSpacing:'0.3px' }}>RELATIVE CONTRAINDICATIONS</div>
            ...
            <div style={{ marginTop:'10px', marginBottom:'6px', fontSize:'8px', color:'var(--c-text-muted)', fontWeight:700, letterSpacing:'0.3px' }}>HISTORY</div>
            ...
            {/* SAR / Weight notice */}
            <div style={{ marginTop:'12px', padding:'8px', background:'var(--c-bg-card)', border:'1px solid var(--c-border)', borderRadius:'2px' }}>
              <div style={{ fontSize:'8px', color:'var(--c-text-subtle)', marginBottom:'4px', fontWeight:700 }}>SAR ESTIMATE (body weight)</div>
              <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
                <div style={{ flex:1, height:'5px', background:'var(--c-bg-elevated)', borderRadius:'2px', overflow:'hidden' }}>
                  <div style={{ height:'100%', width: `${Math.min(100, (Number(form.weight)/120)*100)}%`, background:'var(--c-green)' }} />
                </div>
                <span style={{ fontFamily:'Roboto Mono,monospace', fontSize:'9px', color:'var(--c-text-mid)' }}>{form.weight} kg</span>
              </div>
              <div style={{ fontSize:'8px', color:'var(--c-text-muted)', marginTop:'4px' }}>SAR limits applied automatically based on patient weight and RF pulse configuration.</div>
            </div>

            {/* Keyboard hint */}
            <div style={{ marginTop:'10px', padding:'6px 8px', background:'rgba(34,211,238,0.04)', border:'1px solid rgba(34,211,238,0.1)', borderRadius:'2px', fontSize:'8px', color:'var(--c-text-muted)' }}>
        ...
        {/* ── Footer ─────────────────────────────────────────────────── */}
        <div style={{ padding:'10px 16px', borderTop:'1px solid var(--c-border)', display:'flex', gap:'8px', alignItems:'center', flexShrink:0, background:'var(--c-bg-dark)' }}>
          <span style={{ fontSize:'8px', color:'var(--c-text-muted)', fontStyle:'italic', flex:1 }}>
          ...
          <button onClick={() => { setForm(f => ({...f, lastName:'', firstName:'', patientId:'', dob:'', age:'', emergencyContact:''})); setSafeLocal({implant:false,pacemaker:false,pregnant:false,contrastAllergy:false,claustrophobia:false,previousMRI:false,emergencyContact:''}); }}
            style={{ fontSize:'9px', background:'transparent', border:'1px solid var(--c-border-bright)', color:'var(--c-text-subtle)', padding:'5px 12px', cursor:'pointer', borderRadius:'2px' }}>
            Clear
          </button>
          <button onClick={togglePatient}
            style={{ fontSize:'9px', background:'transparent', border:'1px solid var(--c-border-bright)', color:'var(--c-text-mid)', padding:'5px 16px', cursor:'pointer', borderRadius:'2px' }}>
            Cancel
          </button>
          <button onClick={handleSave} tabIndex={15}
            style={{ fontSize:'10px', fontWeight:700, background:'rgba(34,211,238,0.15)', border:'1px solid rgba(34,211,238,0.4)', color:'var(--c-cyan)', padding:'5px 24px', cursor:'pointer', borderRadius:'2px' }}>
            Register Patient ▶
          </button>
        </div>
      </div>
