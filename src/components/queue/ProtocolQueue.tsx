...
const STATUS_CONFIG = {
  done:     { color:'var(--c-green)', icon:'✓', label:'DONE' },
  active:   { color:'var(--c-amber)', icon:'◉', label:'ACTIVE' },
  scanning: { color:'var(--c-cyan)', icon:'▶', label:'SCAN' },
  pending:  { color:'var(--c-text-muted)', icon:'○', label:'WAIT' },
} as const;
...
const ContextMenu = memo(function ContextMenu({
  ...
  const items = [
    ...
    null, // separator
    { label:'✕ Delete',         fn: () => { onDelete(menu.seqId);    onClose(); }, color:'var(--c-red)' },
  ];
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background:'var(--c-bg-card)', border:'1px solid var(--c-border-bright)', borderRadius:'3px',
        boxShadow:'0 4px 20px rgba(0,0,0,0.6)', minWidth:'140px', overflow:'hidden',
      }}>
        {items.map((item, i) =>
          item === null
            ? <div key={i} style={{ height:'1px', background:'var(--c-border)', margin:'2px 0' }} />
            : (
              <button
                key={item.label}
                onClick={item.fn}
                style={{
                  ...
                  color: item.color ?? 'var(--c-text-mid)', cursor:'pointer', whiteSpace:'nowrap',
                }}
  ...
  return (
    <div
      ...
      style={{
        ...
        borderBottom: isDragOver ? '2px solid var(--c-cyan)' : '1px solid var(--c-border-faint)',
        cursor:'pointer',
        background: isSelected ? 'var(--c-bg-selected)' : isScanning ? 'rgba(34,211,238,0.04)' : 'transparent',
        borderLeft: isSelected ? '2px solid var(--c-cyan)' : '2px solid transparent',
    ...
    >
      {/* Drag handle */}
      <span style={{ color:'var(--c-border)', fontSize:'10px', cursor:'grab', flexShrink:0, lineHeight:'1' }}>⠿</span>

      <span style={{ fontFamily:'Roboto Mono,monospace', fontSize:'8.5px', color:'var(--c-text-muted)' }}>{idx+1}</span>

      <span style={{
        fontSize:'7.5px', fontWeight:700, letterSpacing:'0.3px',
        color:cfg.color, background:`color-mix(in srgb, ${cfg.color} 8%, transparent)`,
        border:`1px solid color-mix(in srgb, ${cfg.color} 20%, transparent)`, borderRadius:'2px',
      ...
      {series ? (
        <button
          ...
          style={{
            ...
            background:'var(--c-bg-input)', overflow:'hidden', borderRadius:'2px', cursor:'pointer',
          }}
        >
      ...
      <span style={{ display:'flex', alignItems:'center', minWidth:0, gap:'5px' }}>
        <span style={{
          ...
          color: seq.status==='done' ? 'var(--c-text-muted)' : seq.status==='active' ? 'var(--c-text-base)' : 'var(--c-text-mid)',
        ...
        {series && (
          <span style={{
            ...
            color:'var(--c-cyan)', background:'#083344', border:'1px solid #155e75', borderRadius:'2px', padding:'1px 3px',
          }}>{series.frameCount} IMG</span>
        )}
      </span>

      <span style={{ fontFamily:'Roboto Mono,monospace', fontSize:'9px', color:'var(--c-cyan)', textAlign:'right' }}>{seq.ta}</span>
      <span style={{ fontFamily:'Roboto Mono,monospace', fontSize:'9px', color:'var(--c-text-subtle)', textAlign:'right' }}>{seq.sl}</span>
      <span style={{ fontFamily:'Roboto Mono,monospace', fontSize:'9px', color:'var(--c-text-subtle)', textAlign:'right' }}>{seq.tr}</span>
      <span style={{ fontFamily:'Roboto Mono,monospace', fontSize:'9px', color:'var(--c-text-subtle)', textAlign:'right' }}>{seq.te}</span>

      <div>
        {isScanning ? (
          <div style={{ height:'3px', background:'var(--c-bg-elevated)', borderRadius:'2px', overflow:'hidden' }}>
            <div style={{ height:'100%', width:`${scanProgress}%`, background:'var(--c-cyan)', borderRadius:'2px', transition:'width 0.2s', boxShadow:'0 0 4px var(--c-cyan)' }} />
          </div>
        ) : seq.status === 'done' ? (
...
export default function ProtocolQueue() {
  ...
  return (
    <div
      style={{ display:'flex', flexDirection:'column', flex:'1 1 0', minWidth:0, height:'100%', overflow:'hidden', background:'var(--c-bg-card)' }}
      onDragLeave={() => setDragOverId(null)}
    >
      {/* ── Header ────────────────────────────────────────────────────── */}
      <div style={{ display:'flex', alignItems:'center', gap:'4px', padding:'0 6px', height:'30px', borderBottom:'1px solid var(--c-border)', flexShrink:0, background:'var(--c-bg-panel)' }}>
        <span style={{ fontSize:'10px', fontWeight:700, color:'var(--c-text-subtle)', letterSpacing:'0.5px', marginRight:'4px', whiteSpace:'nowrap', flexShrink:0 }}>PROTOCOL QUEUE</span>

        {/* Scan control buttons */}
        {[
          { label:'▶ Run',   color:'var(--c-cyan)', action: startScan,  disabled: scan.running && !scan.paused, title:'F5' },
          { label:'⏸ Pause', color:'var(--c-amber)', action: pauseScan, disabled: !scan.running || scan.paused,  title:'F6' },
          { label:'⏹ Abort', color:'var(--c-red)', action: stopScan,  disabled: !scan.running,                 title:'Esc' },
        ].map(btn => (
          <button
            key={btn.label}
            onClick={btn.action}
            ...
            title={btn.title}
            style={{
              fontSize:'9px', padding:'2px 8px', border: btn.disabled ? '1px solid var(--c-border)' : `1px solid color-mix(in srgb, ${btn.color} 35%, transparent)`,
              background: btn.disabled ? 'transparent' : `color-mix(in srgb, ${btn.color} 8%, transparent)`,
              color: btn.disabled ? 'var(--c-text-muted)' : btn.color,
        ...
        <div style={{ width:'1px', height:'14px', background:'var(--c-border)', margin:'0 2px', flexShrink:0 }} />
        ...
        {[
          ...
          <button key={btn.label} onClick={btn.fn} title={btn.title}
            style={{
              fontSize:'9px', padding:'2px 7px', border:'1px solid var(--c-border)',
              background:'transparent', color: btn.color ?? 'var(--c-text-mid)', cursor:'pointer', borderRadius:'2px', flexShrink:0,
            }}
          >{btn.label}</button>
        ))}
        ...
        {/* Progress info */}
        <span style={{ fontFamily:'Roboto Mono,monospace', fontSize:'8px', color:'var(--c-text-muted)', flexShrink:0 }}>
          {doneCount}/{sequences.length}
        </span>
        <div style={{ fontFamily:'Roboto Mono,monospace', fontSize:'13px', color: scan.running?'var(--c-cyan)':'var(--c-text-muted)', letterSpacing:'1px', minWidth:'44px', flexShrink:0 }}>
          {scan.running ? formatTime(Math.round(scan.remainSec)) : '—'}
        </div>
        <div style={{ width:'80px', height:'4px', background:'var(--c-bg-elevated)', borderRadius:'2px', overflow:'hidden', border:'1px solid var(--c-border)', flexShrink:0 }}>
          <div style={{ height:'100%', width:`${scan.progress}%`, background:'var(--c-cyan)', borderRadius:'2px', transition:'width 0.2s', boxShadow:'0 0 4px var(--c-cyan)' }} />
        </div>
        <span style={{ fontFamily:'Roboto Mono,monospace', fontSize:'9px', color:'var(--c-text-subtle)', minWidth:'28px', flexShrink:0 }}>{Math.round(scan.progress)}%</span>
      </div>
      ...
      <div style={{
        ...
        background:'var(--c-bg-dark)', borderBottom:'1px solid var(--c-border)',
        position:'sticky', top:0, zIndex:5,
        color:'var(--c-text-muted)', fontFamily:'Roboto Mono,monospace', fontSize:'8px', fontWeight:700, letterSpacing:'0.3px',
      ...
      {/* ── Footer ──────────────────────────────────────────────────────── */}
      <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'0 8px', height:'18px', borderTop:'1px solid var(--c-border-faint)', background:'var(--c-bg-dark)', flexShrink:0, fontSize:'8.5px' }}>
        <span style={{ color:'var(--c-text-muted)', fontFamily:'Roboto Mono,monospace' }}>Σ {formatTime(totalSec)}</span>
        <span style={{ color:'var(--c-border)' }}>|</span>
        <span style={{ color:'var(--c-text-muted)' }}>{sequences.length} seq</span>
        <span style={{ color:'var(--c-border)' }}>|</span>
        <span style={{ color:'var(--c-text-muted)' }}>F5=Run · F6=Pause · Esc=Abort · Drag series to viewer · Double-click=Open · Right-click=Menu</span>
        <div style={{ flex:1 }} />
        <span style={{ color:'var(--c-text-muted)', fontFamily:'Roboto Mono,monospace' }}>Calc TA: {calcTA}</span>
      </div>
